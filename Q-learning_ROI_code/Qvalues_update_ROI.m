function [Q_matrix_new] = Qvalues_update_ROI(currentState, nextState, next_action, flagID,  Q_matrix, reward, gamma, lr)
...
Q_value = Q_matrix(currentState(1),currentState(2),next_action,flagID);
Q_target = reward + gamma*max(Q_matrix(nextState(1),nextState(2),:,flagID));

% Update Q matrix
Q_matrix_new(currentState(1),currentState(2),next_action,flagID) = (1-lr)*Q_value + lr*Q_target;
